...
export class SpecOperations {
	...
	async createDecision(
		data: Omit<Decision, "number">,
	): Promise<SpecOperationResult<Decision>> {
		try {
			const decision = await this.manager.createDecision(data);
			return {
				success: true,
				data: decision,
			};
	...
	async getDecision(id: string): Promise<SpecOperationResult<Decision>> {
		try {
			const decision = await this.manager.getDecision(id);
			if (!decision) {
				return {
					success: false,
	...
	async updateDecision(
		...
		try {
			const decision = await this.manager.updateDecision(id, data);
			return {
				success: true,
				data: decision,
			};
	...
	async deleteDecision(id: string): Promise<SpecOperationResult<boolean>> {
		try {
			const deleted = await this.manager.deleteDecision(id);
			return {
				success: true,
				data: deleted,
			};
	...
	async listDecisions(
		...
		try {
			const decisions = await this.manager.listDecisions(filter);
			return {
				success: true,
				data: decisions,
			};
	...
	> {
		try {
			...
			try {
				decisions = await this.manager.listDecisions();
			} catch (decisionError) {
				// Decisions are optional - if the manager doesn't support them, return empty array
				console.warn("Failed to load decisions:", decisionError);
			}
			return {
				success: true,
				data: {
