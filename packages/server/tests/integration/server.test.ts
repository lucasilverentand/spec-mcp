...
import { InputValidator } from "../../src/middleware/input-validator.js";
...
describe("Server Configuration", () => {
	it("should load configuration from environment", async () => {
		process.env.SPECS_ROOT = "./test-specs";
		...
		expect(config.schemaValidation).toBe(true);
	});

	it("should create specs directory if it doesn't exist", async () => {
		process.env.SPECS_ROOT = "./test-specs-new";
		const config = await loadConfig();
		expect(config.specsPath).toContain("test-specs-new");
	});

	it("should validate configuration schema", async () => {
		const originalSpecsRoot = process.env.SPECS_ROOT;
		const originalMaxFileSize = process.env.MAX_FILE_SIZE;

		// Test with invalid max file size (negative number)
		process.env.MAX_FILE_SIZE = "-1000";

		await expect(loadConfig()).rejects.toThrow(McpError);

		// Restore
		process.env.SPECS_ROOT = originalSpecsRoot;
		process.env.MAX_FILE_SIZE = originalMaxFileSize || "";
	});
});
