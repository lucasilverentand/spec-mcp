...
import { canCompleteTask, canStartTask, validateEntity } from "@spec-mcp/core";
import type { Plan } from "@spec-mcp/schemas";

/**
 * Start a task by marking it as started (no git operations)
 */
...
/**
 * Finish a task by marking it as completed
 */
export async function finishTaskGit(
	...
	try {
		...
		const now = new Date().toISOString();
		const completionNote = `Completed at ${now}: ${summary}`;
...
export const finishTaskGitTool = {
	name: "finish_task",
	description:
		"Mark a task as completed by adding a timestamped note with the summary. Will fail if dependencies are not completed.",
	inputSchema: {
		type: "object",
		properties: {
			plan_id: {
